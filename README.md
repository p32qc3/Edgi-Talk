...
- `M55_Edgi_Pet_A1`：宠物界面与“通信 + AI”整合工程；原 M55 工程保持不变，便于回退。
- `pet_shared`：M33、M55 共用的存储协调定义。
- `tests`：通信可靠性、AI 指令限制和状态显示的电脑端自动检查。
- `模型与验证`：当前模型及离线验证结果。
- `现场工具`：电脑端 WiFi 配网工具。
...
宠物整合版必须先烧录 M33，再烧录 `M55_Edgi_Pet_A1`。详细步骤、51 接线和验收方法见 [A+B 整合版构建与烧录说明](docs/AB1_BUILD_AND_FLASH.md)。
